# -*- coding: utf-8 -*-
"""solar_pv_census_data_analysis_E2E.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/github/avega17/CCOM6994_PV_Solar_Project/blob/main/solar_pv_census_data_analysis_E2E.ipynb
...
### 🔧 Interactive LULC Processing & Visualization

This section provides:
1. **State Selection Widget**: Choose a US state to process LULC data
2. **Processing**: Triggers `lulc_state_extract.py` to fetch Overture Maps data for the state
3. **Visualization**: Display land cover and land use distributions for processed states

The LULC data is persisted in DuckDB (`lulc_enriched_pv_data` table) for reuse across analyses.

**Usage**:
- Select a state from the dropdown
- Click "Procesar LULC" to fetch and join LULC data (takes 2-10 minutes per state)
- Use "Ver Land Cover" and "Ver Land Use" buttons to visualize results

**Note**: Processing each state is required only once. Results are cached in the database.
"""
...
def process_lulc_for_state(state_fips):
    """Run lulc_state_extract.py for the selected state."""
    with status_output:
        clear_output(wait=True)
        print(f"🔄 Procesando datos LULC para estado {state_fips}...")
        print("   Este proceso puede tomar varios minutos dependiendo del tamaño del estado...")
        
        try:
            # Run the LULC extraction script
            result = subprocess.run(
                ['python', f'{local_path}notebooks/lulc_state_extract.py', '--state', state_fips],
                capture_output=True,
                text=True,
            ...
            print("\n⏱️ Timeout: El procesamiento tomó demasiado tiempo")
        except Exception as e:
            print(f"\n❌ Error: {str(e)}")

def on_process_button_clicked(b):
    """Handle process button click."""
    state_fips = state_dropdown.value
    process_lulc_for_state(state_fips)

process_button.on_click(on_process_button_clicked)

# Display widgets
print("\n💡 Seleccione un estado y haga clic en 'Procesar LULC' para extraer datos de cobertura y uso de tierra")
display(HBox([state_dropdown, process_button]))
display(status_output)

"""### 📊 Visualización de Land Cover

Utilizando lonboard para visualización interactiva de los datos de cobertura de tierra procesados.
"""

# Create visualization widget
viz_state_dropdown = Dropdown(
    options=state_options,
...
viz_lc_button = Button(
    description='Ver Land Cover',
    button_style='info',
    layout=Layout(width='200px')
)

viz_lu_button = Button(
